		...
		<?endif?>
	      ...
	      <div class="modal-body">
			  <div class="mb-3">
			    ...
			    <input type="text" name="UF_TITLE" class="form-control" id="todoListElementNewTitle" required value="<?=$element['UF_TITLE'] ?? ''?>">
			  </div>
			  <div class="mb-3">
			    <label for="todoListElementNewDescription" class="form-label">Описание</label>
			    <textarea class="form-control" name="UF_DESCRIPTION" id="todoListElementNewDescription" rows="2" required><?=$element['UF_DESCRIPTION'] ?? ''?></textarea>
			  </div>
			 <div class="col-md-6 mb-3">
				...
				<?if($element['UF_TAGS'] && count($element['UF_TAGS'])):?>
					<?foreach($element['UF_TAGS'] as $tag):?>
						<input type="text" name="UF_TAGS[]" class="form-control form-control-sm mb-2 js-button-tags" aria-describedby="todoListElementNewTagsHelp" value="<?=$tag?>">
					<?endforeach?>
				<?else:?>
					<input type="text" name="UF_TAGS[]" class="form-control form-control-sm mb-2 js-button-tags" aria-describedby="todoListElementNewTagsHelp">
				<?endif?>
